...
- [重载(Overload)和重写(Override)](#%E9%87%8D%E8%BD%BDoverload%E5%92%8C%E9%87%8D%E5%86%99override)
- [<font size="5">静态成员和常量</font>](#font-size%225%22%E9%9D%99%E6%80%81%E6%88%90%E5%91%98%E5%92%8C%E5%B8%B8%E9%87%8Ffont)
    - [**static**](#static)
    - [**static const**](#static-const)
    - [**const**](#const)
- [<font size="5">静态链接和动态链接</font>](#font-size%225%22%E9%9D%99%E6%80%81%E9%93%BE%E6%8E%A5%E5%92%8C%E5%8A%A8%E6%80%81%E9%93%BE%E6%8E%A5font)
...
重载：同一个方法对于不同参数的处理；函数名相同，参数列表不同（类型/数量）。

重写：子类对父类函数的重新定义，体现多态；函数名和参数列表都相同。
...
---
# <font size="5">静态链接和动态链接</font>

### 静态链接

多个源文件编译之后形成多个obj文件，obj文件之间可能存在依赖关系。把存在依赖关系的obj文件里面，复制需要的函数和数据到可执行文件中，链接在一起。

比如，main.obj调用了printf函数，则main.obj会从包含printf函数的obj文件中，把printf函数相关的二进制代码复制到可执行文件中，链接在一起。

优点：

* 可执行程序中已经有了执行程序所需的所有东西，不需要依赖库，运行速度也会更快；

缺点：

* 如果多个源文件调用了同一个函数，这个函数会在内存中有多个副本；

* 更新较困难，如果库函数的代码修改了，需要重新编译、链接，形成可执行程序；

### 动态链接

在编译过程中，不直接拷贝可执行代码，而是通过记录一系列符号和参数，在程序运行或加载时将这些信息传递给操作系统，操作系统负责将需要的动态库加载到内存中，然后程序在运行到指定的代码时，去共享执行内存中已经加载的动态库可执行代码，最终达到运行时连接的目的。

比如，program1.obj被加载，系统发现它依赖lib.obj，于是加载到内存；之后加载program2.obj，发现也依赖于lib.obj，这时不需要重新加载，而是将内存中已经存在的lib.obj映射到program2的虚拟地址空间中，从而进行链接形成可执行程序。
...
* 程序执行时需要链接，需要依赖库，并且性能会有损失；

[静态链接和动态链接](https://blog.csdn.net/kang___xi/article/details/80210717)

[从源代码到运行](https://blog.csdn.net/kang___xi/article/details/79571137)
